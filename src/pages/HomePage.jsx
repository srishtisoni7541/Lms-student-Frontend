...
const Homepage = () => {
  ...
  const features = [
    ...
    { icon: BookOpen, title: "Course Materials", desc: "Downloadable resources" },
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-indigo-50 via-white to-purple-50">
      ...
      {/* ===== Hero Section ===== */}
      <section className="py-76 sm:py-20 px-4 sm:px-6 lg:px-8 text-center">
        ...
        <p className="text-lg sm:text-xl text-gray-600 mb-8 mx-auto max-w-xl sm:max-w-2xl">
          Join thousands of students learning from industry experts. Build real projects, earn certificates, and advance your career.
        </p>
        <div className="flex flex-wrap justify-center gap-3 sm:gap-4 mb-12">
          {features.map((feature, idx) => (
            <div key={idx} className="flex items-center bg-white/60 backdrop-blur-sm rounded-full px-4 sm:px-6 py-2 sm:py-3 border border-gray-200">
      ...
      <section className="py-12 sm:py-16 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12 sm:mb-16">
            <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-gray-900 mb-3">Popular Courses</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
              {courses.map((course) => (
                <div
                  ...
                  className="group bg-white rounded-2xl overflow-hidden shadow-lg border border-gray-200 hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 cursor-pointer"
                >
                  <div className="relative overflow-hidden">
                    <img
                      src={course.thumbnail}
                      alt={course.title}
                      className="w-full h-40 sm:h-48 md:h-48 object-cover group-hover:scale-110 transition-transform duration-300"
                    />
                    <div className="absolute top-3 left-3 sm:top-4 sm:left-4">
                      <span className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium">
                        {course.category}
                      </span>
                    </div>
                  </div>
                  <div className="p-4 sm:p-6">
                    ...
                    {isCourseBought(course._id) ? (
                      <button
                        onClick={() => navigate(`/course/${course._id}`)}
                        className="w-full mt-2 sm:mt-4 bg-green-600 text-white py-2 sm:py-3 px-4 sm:px-6 rounded-lg font-semibold hover:bg-green-700 transition-all transform hover:scale-105"
                      >
                        View
                      </button>
                    ) : (
                      <button
                        ...
                        disabled={!user}
                        className="w-full mt-2 sm:mt-4 bg-gradient-to-r from-indigo-600 to-purple-600 text-white py-2 sm:py-3 px-4 sm:px-6 rounded-lg font-semibold hover:from-indigo-700 hover:to-purple-700 transition-all transform hover:scale-105 focus:outline-none focus:ring-2 sm:focus:ring-4 focus:ring-indigo-300"
                      >
                        Buy Now
                      </button>
                    )}
      ...
      <footer className="bg-gray-900 text-white py-8 sm:py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8">
            ...
            <div>
              <h3 className="text-lg font-semibold mb-2 sm:mb-4">Support</h3>
              <ul className="space-y-1 sm:space-y-2 text-gray-400 text-sm sm:text-base">
          ...
          <div className="border-t border-gray-800 mt-6 sm:mt-8 pt-4 text-center text-gray-400 text-sm sm:text-base">
